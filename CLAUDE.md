...
**One-command dev workflow** (a `Makefile` wraps the raw commands; see `RUN.md`): `make setup` (once)
→ `make demo` (mock data) / `make live` (real Shopify store, source `live_rest`) → `make run` (app).
`make reset` wipes crawled data; `make help` lists targets. `make test` shares the dev DB and
**truncates** data (re-crawl after). Dev `.env` is generated by `make setup` (git-ignored). Raw:

```bash
uv sync                              # install deps into the venv
uv run scrapy crawl <spider>         # run one spider
uv run acquire-intel crawl <source>  # CLI wrapper (on-demand crawl); sources: demo_rest/live_rest/…
uv run python scripts/demo_seed.py --scenario <s>   # seed demo_rest at a harness scenario
uv run python scripts/seed_live.py --url <store>    # seed live_rest at a real Shopify store
...
uv run python -m harness.server      # start the adversarial mock server (--block-after N to force rotation)
```
...
### Post-roadmap polish (merged to `main`)

The roadmap is done; these are portfolio/DX additions on top, each its own PR:
- **One-command dev workflow** (PR #7): `Makefile` (`make setup/demo/run/reset/…`) + a git-ignored
  dev `.env` generator, so running the app matches "one command" muscle memory. See `RUN.md`.
- **Real live store** (PR #8): `LiveRestExtractor` (source id `live_rest`, distinct from the mock's
  `demo_rest`) crawls any public Shopify `products.json` through the same engine — verified against
  deathwishcoffee.com (132 real products, `robots.txt` obeyed). `make demo` ⟷ `make live` switch.

**Known follow-ups (not yet done):** (1) point the test suite at a separate DB — its integration
tests `TRUNCATE` and currently share the dev database, so `make test` wipes crawled dev data;
(2) graceful DB-down handling — data routes/dashboard return a raw 500 when Postgres is unreachable
(should be a 503 like `/health` already is).
